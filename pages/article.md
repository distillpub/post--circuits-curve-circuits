...
How do we know this story about the mechanics of curve detectors is true? One way is to use it to reimplement curve detectors from scratch. We manually set the weights of a blank neural network to implement the neuron families and circuit motifs from this article, crafting an <i>artificial</i> artificial neural network, and made the python code available and runnable in <a href="https://colab.research.google.com/drive/1WIazW0e7I-ZOFqTWzUUN7wWtetXrbwCA">this Colab notebook</a>. This was initially a few hour process for one person (Chris Olah), and they did not look at the original neural network’s weights when constructing it, which would go against the spirit of the exercise. Later, before publishing they tweaked the weights and in particular added negative weights to remove echoes in the activations.
